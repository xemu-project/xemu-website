...
float getGradients(vec2 uv)
{
    ...
    float gradient_y = smoothstep(0.6, 1., 1.-abs(0.5-uv.y));
...
float getSpotlights(vec2 uv)
{
    ...
    float right = smoothstep(0.3, 1., 1.2-abs((gw + (t - uv.x + textPos.x - lineWidth/2.))/(lineWidth/0.7)));
...
vec2 getParticlePosition(int i)
{
    ...
    velocity.x = 0.4*impulse*sign(random(float(i+66)) - 0.2)*(0.3 + 0.6*random(float(i + 100)));
